...
/* inline script 3 */
(function () {
  const items = document.querySelectorAll('.ti-acc__item');
  if (!items.length) return;

  items.forEach(item => {
    const btn = item.querySelector('.ti-acc__header');
    btn.addEventListener('click', () => {
      items.forEach(i => {
        if (i !== item) i.classList.remove('ti-acc__item--open');
      });
      item.classList.toggle('ti-acc__item--open');
    });
  });
})();
...
(function(){
    ...
    // Открываем по любому клику на ссылку с href="#consult"
    const openBtns = document.querySelectorAll('a[href="#consult"]');
...
/* inline script 8 */
/* аккордеон только для планшетного блока */
(function () {
  const items = document.querySelectorAll('.ti-tab-acc__item');
  if (!items.length) return;

  items.forEach(item => {
    const btn = item.querySelector('.ti-tab-acc__header');
    btn.addEventListener('click', () => {
      items.forEach(i => {
        if (i !== item) i.classList.remove('ti-tab-acc__item--open');
      });
      item.classList.toggle('ti-tab-acc__item--open');
    });
  });
})();
...
/* inline script 13 */
/* аккордеон для мобильного блока */
(function () {
  const items = document.querySelectorAll('.ti-mob-acc__item');
  if (!items.length) return;

  items.forEach(item => {
    const btn = item.querySelector('.ti-mob-acc__header');
    btn.addEventListener('click', () => {
      items.forEach(i => {
        if (i !== item) i.classList.remove('ti-mob-acc__item--open');
      });
      item.classList.toggle('ti-mob-acc__item--open');
    });
  });
})();

/* логика модалки (мобильный блок) */
(function(){
  const modal   = document.getElementById('ti-mob-modal');
  if (!modal) return;

  const backdrop = modal.querySelector('.ti-mob-modal__backdrop');
  const closeBtn = modal.querySelector('.ti-mob-modal__close');

  // кнопки открытия: только кнопка из мобильного hero
  const openBtns = document.querySelectorAll('.ti-mob-hero__btn');
