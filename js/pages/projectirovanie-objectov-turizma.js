...
/* Параллакс-логотип */
(function(){
  var root = document.querySelector('.projx');
  var brand = document.querySelector('.px-brand--harmony');
  if(!root || !brand) return;
  var speed = -0.5;
  function tick(){
    var y = -root.getBoundingClientRect().top;
    brand.style.setProperty('--px-shift', (y * speed) + 'px');
    requestAnimationFrame(tick);
  }
  requestAnimationFrame(tick);
})();

/* Модалка "Проконсультироваться" */
(function(){
  ...
  var triggers = document.querySelectorAll('.projx .px-btn--dark');
...
(function(){
  ...
  const brand=document.querySelector('.proj-tab__brand--harmony');
  if(!root||!brand) return;
...
/* inline script 12 */
(function(){
  const root = document.querySelector('.proj-mob');
  const brand = document.querySelector('.pm-brand');
  if(!root || !brand) return;
  const speed = -0.5;
  function tick(){
    const y = -root.getBoundingClientRect().top;
...
/* inline script 13 */
/* Открытие/закрытие imccm по клику на "Проконсультироваться" (моб+десктоп) */
(function(){
  ...
  // Работает для:
  // - кнопок .pm-btn внутри .proj-mob (мобильный туризм)
  // - кнопок .px-btn--dark внутри .projx (десктопный туризм)
  // - старых .im-btn внутри .im-proj (если есть)
  document.addEventListener('click', function(e){
    const btn = e.target.closest('.proj-mob .pm-btn, .projx .px-btn--dark, .im-proj .im-btn');
    if (!btn) return;
    openModal(e);
  }, { passive:false });
