...
// аккордеон
  document.addEventListener('click', function(e){
    const header = e.target.closest('.step__header');
    if (!header) return;
    ...
    steps.forEach(s => {
      s.classList.remove('step--open');
      const b = s.querySelector('.step__body');
  ...
  (function(){
    ...
    // все кнопки, которые должны открывать модалку:
    const openBtns = document.querySelectorAll('.proj-btn--dark, .js-open-consult');
...
/* inline script 7 */
// аккордеон с уникальными классами
  document.addEventListener('click', function(e){
    const tscHeader = e.target.closest('.tsc-step-header');
    if (!tscHeader) return;

    const tscStep = tscHeader.closest('.tsc-step');
    const tscBody = tscStep.querySelector('.tsc-step-body');
    const allSteps = tscStep.parentElement.querySelectorAll('.tsc-step');
    const isOpen = tscStep.classList.contains('tsc-step-open');

    allSteps.forEach(function(step){
      step.classList.remove('tsc-step-open');
      const body = step.querySelector('.tsc-step-body');
      if (body) body.style.maxHeight = 0;
    });

    if (!isOpen){
      tscStep.classList.add('tsc-step-open');
      if (tscBody) tscBody.style.maxHeight = tscBody.scrollHeight + 'px';
    }
  });

  // модалка с уникальными классами
  (function(){
    const modal = document.getElementById('tsc-modal');
    if (!modal) return;

    const backdrop = modal.querySelector('.tsc-modal-backdrop');
    const closeBtn = modal.querySelector('.tsc-modal-close');
    const openBtns = document.querySelectorAll('.tsc-open-consult');
...
// аккордеон
  document.addEventListener('click', function(e){
    const header = e.target.closest('.mcx-step-header');
    if (!header) return;

    const step = header.closest('.mcx-step');
    const body = step.querySelector('.mcx-step-body');
    const allSteps = step.parentElement.querySelectorAll('.mcx-step');
    const isOpen = step.classList.contains('mcx-step-open');

    allSteps.forEach(function(s){
      s.classList.remove('mcx-step-open');
      const b = s.querySelector('.mcx-step-body');
      if (b) b.style.maxHeight = 0;
    });

    if (!isOpen){
      step.classList.add('mcx-step-open');
      if (body) body.style.maxHeight = body.scrollHeight + 'px';
    }
  });
  ...
  // модалка
  (function(){
    const modal = document.getElementById('mcx-modal');
    if (!modal) return;

    const backdrop = modal.querySelector('.mcx-modal-backdrop');
    const closeBtn = modal.querySelector('.mcx-modal-close');
    const openBtns = document.querySelectorAll('.mcx-open-modal');
