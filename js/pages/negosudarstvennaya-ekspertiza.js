...
/* inline script 3 */
(function(){
  const accordion = document.querySelector('.pdii-accordion');
  if (!accordion) return;

  const items = accordion.querySelectorAll('.pdii-acc-item');

  items.forEach(item => {
    const header = item.querySelector('.pdii-acc-header');
    const body   = item.querySelector('.pdii-acc-body');

    // все пункты закрыты по умолчанию
    body.style.maxHeight = 0;

    header.addEventListener('click', () => {
      const isOpen = item.classList.contains('pdii-acc-item--open');

      items.forEach(i => {
        i.classList.remove('pdii-acc-item--open');
        const b = i.querySelector('.pdii-acc-body');
        b.style.maxHeight = 0;
      });

      if (!isOpen) {
        item.classList.add('pdii-acc-item--open');
...
(function(){
  ...
  // Кнопки, которые открывают модалку: "Проконсультироваться" и "Связаться"
  const openBtns = document.querySelectorAll('.pdii-objects__btn, .pdii-hero__btn');
...
(function(){
  /* аккордеон */
  const tabAccordion = document.querySelector('.pdii-tab-accordion');
  if (tabAccordion) {
    const tabItems = tabAccordion.querySelectorAll('.pdii-tab-acc');

    tabItems.forEach(item => {
      const head = item.querySelector('.pdii-tab-acc__head');
      const body = item.querySelector('.pdii-tab-acc__body');

      body.style.maxHeight = 0; // все закрыты

      head.addEventListener('click', () => {
        const isOpen = item.classList.contains('pdii-tab-acc--open');

        tabItems.forEach(el => {
          el.classList.remove('pdii-tab-acc--open');
          const b = el.querySelector('.pdii-tab-acc__body');
          b.style.maxHeight = 0;
        });

        if (!isOpen) {
          item.classList.add('pdii-tab-acc--open');
          body.style.maxHeight = body.scrollHeight + 'px';
        }
      });
    });
  ...
  /* модалка ptccm для планшетного блока */
  ...
  const openBtns = document.querySelectorAll('.pdii-tab-hero__btn, .pdii-tab-objects__btn');
...
(function(){
  /* аккордеон на мобиле */
  const mobAcc = document.querySelector('.pdii-mob-accordion');
  if (mobAcc){
    const items = mobAcc.querySelectorAll('.pdii-mob-acc');

    items.forEach(item => {
      const head = item.querySelector('.pdii-mob-acc__head');
      const body = item.querySelector('.pdii-mob-acc__body');

      body.style.maxHeight = 0; // всё закрыто

      head.addEventListener('click', () => {
        const isOpen = item.classList.contains('pdii-mob-acc--open');

        items.forEach(el => {
          el.classList.remove('pdii-mob-acc--open');
          const b = el.querySelector('.pdii-mob-acc__body');
          b.style.maxHeight = 0;
        });

        if (!isOpen){
          item.classList.add('pdii-mob-acc--open');
          body.style.maxHeight = body.scrollHeight + 'px';
        }
      });
    });
  ...
  /* Открытие/закрытие imccm по клику на mobile-кнопки блока pdii-mob */
  ...
  // Клик по "Связаться" и "Проконсультироваться" в mobile-блоке
  document.addEventListener('click', function(e){
    const btn = e.target.closest('.pdii-mob__btn, .pdii-mob-objects__btn');
    if (!btn) return;
    e.preventDefault();
    openModal();
  }, { passive:false });
