...
(function(){
  ...
  const brand=document.querySelector('.proj__brand');
...
(function(){
  ...
  function openModal(){modal.setAttribute('open','');modal.removeAttribute('aria-hidden');document.documentElement.style.overflow='hidden'}
  function closeModal(){modal.removeAttribute('open');modal.setAttribute('aria-hidden','true');document.documentElement.style.overflow=''}
...
/* inline script 8 */
(function(){
    const root=document.querySelector('.proj-tab');
    const brand=document.querySelector('.proj-tab__brand');
    if(!root||!brand)return;
    const speed=-0.5;
    function tick(){
      const y=-root.getBoundingClientRect().top;
...
(function(){
    ...
    const openBtns=document.querySelectorAll('.proj-tab-btn');
    function open(){modal.setAttribute('open','');modal.setAttribute('aria-hidden','false');document.documentElement.style.overflow='hidden'}
    function closeIt(){modal.removeAttribute('open');modal.setAttribute('aria-hidden','true');document.documentElement.style.overflow=''}
...
(function(){
    ...
    document.addEventListener('click',function(e){
      const btn=e.target.closest('.im-proj .im-btn');
      if(!btn)return;
      e.preventDefault();
      openModal();
    },{passive:false});
...
/* Parallax */
(function(){
  var root=document.querySelector('.ax-proj');
  var brand=document.querySelector('.ax-brand');
  if(!root||!brand) return;
  var speed=-0.5;
  function tick(){
    var y=-root.getBoundingClientRect().top;
    brand.style.setProperty('--ax-shift',(y*speed)+'px');
    requestAnimationFrame(tick);
  }
  requestAnimationFrame(tick);
})();

/* Robust modal for WP (unique names + attribute [hidden]) */
(function(){
  function onReady(fn){
  ...
  onReady(function(){
    ...
    var closeTriggers = modal.querySelectorAll('[data-ax-close], .ax-modal__backdrop');
    var openAttr = 'data-ax-open';
    var lastOpener = null;
    ...
    function openModal(opener){
      ...
      // focus
      setTimeout(function(){
        var focusable = panel.querySelector('a,button,[tabindex]:not([tabindex="-1"]),input,select,textarea');
    ...
    function onKey(e){
      ...
      if(e.key==='Tab'){
        var focusables = panel.querySelectorAll('a[href],button:not([disabled]),[tabindex]:not([tabindex="-1"]),input,select,textarea');
    ...
    // Open (event delegation so WP shortcodes or cached HTML still work)
    document.addEventListener('click', function(e){
      var opener = e.target.closest('['+openAttr+']');
      if(!opener) return;
      e.preventDefault();
      openModal(opener);
    });
    ...
    // Prevent bubbling close when clicking inside the panel
    panel.addEventListener('click', function(e){ e.stopPropagation(); });

    // Also close if clicking outside panel (on modal root)
