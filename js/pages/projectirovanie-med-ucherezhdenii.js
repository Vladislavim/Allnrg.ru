...
(function(){
  ...
  const brands = Array.from(document.querySelectorAll('.proj__brand'));
  if(!root || !brands.length) return;
  const speed = -0.5;
  function tick(){
    const y = -root.getBoundingClientRect().top;
    brands.forEach(b => b.style.setProperty('--shift', (y * speed) + 'px'));
...
(function(){
  ...
  function padPX(){
    const v = getComputedStyle(document.documentElement)
      .getPropertyValue('--stripe-pad').trim();
...
/* динамическая высота жёлтой полосы под размер текстовой карточки */
(function(){
