...
/* inline script 5 */
(function(){
  const modal = document.getElementById('consult-modal');               // ← верный ID
  if (!modal) return;

  const backdrop = modal.querySelector('.consult-modal__backdrop');     // ← верные классы
  const closeBtn = modal.querySelector('.consult-modal__close');
  const root     = document.querySelector('.proj');                      // ← корень секции

  function openModal(){
    modal.setAttribute('open','');               // показывает по CSS [open]
    modal.removeAttribute('aria-hidden');        // на случай глобального [aria-hidden]{display:none}
  ...
  // Делегирование: клик по любой .proj-btn внутри .proj
...
/* Параллакс логотипа */
(function(){
  var root = document.querySelector('[data-sport-intro]');
  var brand = root ? root.querySelector('.sport-intro__brandmark') : null;
  if(!root || !brand) return;
  var speed = -0.5;
  function loop(){
    var rect = root.getBoundingClientRect();
    var offset = -rect.top * speed;
    brand.style.setProperty('--shift', offset + 'px');
    requestAnimationFrame(loop);
  }
  requestAnimationFrame(loop);
})();

/* Модальное окно: открытие с кнопок .sport-card__btn */
(function(){
  function onReady(fn){
    if(document.readyState === 'loading'){
      document.addEventListener('DOMContentLoaded', fn, { once:true });
  ...
  onReady(function(){
    var modal = document.getElementById('sport-contact-modal');
    if(!modal) return;

    var backdrop = modal.querySelector('.sport-contact__backdrop');
    var closeBtn = modal.querySelector('.sport-contact__close');
    var triggers = document.querySelectorAll('.sport-intro .sport-card__btn');
