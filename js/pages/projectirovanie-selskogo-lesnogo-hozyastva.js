...
/* динамическая высота жёлтой полосы под размер текста */
(function(){
  const rows=Array.from(document.querySelectorAll('.proj-row'));
  if(!rows.length)return;

  const getPad=()=> {
    const v=getComputedStyle(document.documentElement).getPropertyValue('--stripe-pad').trim();
    const n=parseFloat(v); return Number.isFinite(n)?n:24;
  };

  function recalc(){
    const pad=getPad();
    rows.forEach(row=>{
      const text=row.querySelector('.proj-card--text');
      if(!text) return;
      const rowRect=row.getBoundingClientRect();
      const textRect=text.getBoundingClientRect();
      const top=Math.max(0,textRect.top-rowRect.top-pad/2);
      const h=textRect.height+pad;
      row.style.setProperty('--stripe-top',`${top}px`);
      row.style.setProperty('--stripe-h',`${h}px`);
    });
  }
  ...
  rows.forEach(r=>{ ro.observe(r); const t=r.querySelector('.proj-card--text'); if(t) ro.observe(t); });
})();
...
(function(){
  ...
  const openBtns = document.querySelectorAll('.proj-btn--dark'); // все "Проконсультироваться"
...
/* динамическая высота жёлтой полосы – центрируем по карточке с текстом */
(function(){
  const rows=[...document.querySelectorAll('.proj-tab-row')];
  if(!rows.length) return;

  function padPX(){
    const raw=getComputedStyle(document.documentElement).getPropertyValue('--tab-stripe-pad').trim();
  ...
  function recalc(){
    const pad=padPX();
    rows.forEach(row=>{
      const card=row.querySelector('.proj-tab-card--text');
      if(!card) return;
      const rowRect=row.getBoundingClientRect();
      const cardRect=card.getBoundingClientRect();
      const top=Math.max(0, cardRect.top - rowRect.top - pad/2);
      const h=cardRect.height + pad;

      row.style.setProperty('--tab-stripe-top', `${top}px`);
      row.style.setProperty('--tab-stripe-h',   `${h}px`);
    });
  }
...
(function(){
  ...
  const openBtns = document.querySelectorAll('.proj-tab-btn'); // все "Проконсультироваться" в proj-tab
...
/* inline script 14 */
(function(){
  const root=document.querySelector('.im-proj');
  const brand=document.querySelector('.im-brand');
  if(!root||!brand)return;
  const speed=-0.5;
  function tick(){
    const y=-root.getBoundingClientRect().top;
...
/* inline script 15 */
/* Открытие/закрытие imccm по клику на "Проконсультироваться" в .im-proj */
(function(){
