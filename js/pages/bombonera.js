...
/* inline script 3 */
(function(){
  const slider = document.querySelector('.case-ko__slider');
  if (!slider) return;

  const track = slider.querySelector('.case-ko__slider-track');
  const slides = Array.from(slider.querySelectorAll('.case-ko__slide'));
  const prevBtn = slider.querySelector('.case-ko__nav--prev');
  const nextBtn = slider.querySelector('.case-ko__nav--next');
  const dotsWrap = slider.querySelector('.case-ko__dots');
  ...
  if (prevBtn) prevBtn.addEventListener('click', () => { go(index - 1); autoplay(); });
  if (nextBtn) nextBtn.addEventListener('click', () => { go(index + 1); autoplay(); });
...
(function(){
  const frame = document.querySelector('.case-ko__video-frame');
  if (!frame) return;
  const video = frame.querySelector('.case-ko__video-el');
  const playBtn = frame.querySelector('.case-ko__video-play');
...
(function(){
  const strip = document.querySelector('.case-ko__gallery-strip');
  const items = strip ? Array.from(strip.querySelectorAll('.case-ko__gallery-item')) : [];
  const lightbox = document.querySelector('.case-ko__lightbox');
  if (!strip || !items.length || !lightbox) return;

  const imgEl = lightbox.querySelector('img');
  const closeBtn = lightbox.querySelector('.case-ko__lightbox-close');
  const backdrop = lightbox.querySelector('.case-ko__lightbox-backdrop');
...
(function(){
  ...
  function bestThumb(id, cb){
    ...
    (function next(i){
      ...
      const img=new Image();
      img.onload=()=>{ if(img.naturalWidth>=480||i===list.length-1) cb(url); else next(i+1); };
  ...
  document.querySelectorAll('.video-box').forEach(box=>{
    ...
    const raw=box.dataset.yt||box.dataset.ytUrl||box.getAttribute('data-yt-url');
    const id=ytId(raw);
    if(!id||!thumb) return;
    bestThumb(id,u=>{ thumb.src=u; });
    const open=()=>{
      if(box.querySelector('iframe,video')) return;
    ...
    btn&&btn.addEventListener('click',open,{passive:true});
    box.addEventListener('click',e=>{ if(e.target.closest('.video-play')) return; open(); },{passive:true});
  });
})();
...
/* inline script 7 */
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('.eh-tab').forEach(function(block){
    /* SLIDER */
    (function(){
      const slider   = block.querySelector('.eh-tab__slider');
      if (!slider) return;

      const track    = slider.querySelector('.eh-tab__slider-track');
      const slides   = Array.from(slider.querySelectorAll('.eh-tab__slide'));
      const prevBtn  = slider.querySelector('.eh-tab__nav--prev');
      const nextBtn  = slider.querySelector('.eh-tab__nav--next');
      const dotsWrap = slider.querySelector('.eh-tab__dots');
    ...
    /* GALLERY + LIGHTBOX */
    (function(){
      const strip    = block.querySelector('.eh-tab__gallery-strip');
      const lightbox = block.querySelector('.eh-tab__lightbox');
      if (!strip || !lightbox) return;

      const items    = Array.from(strip.querySelectorAll('.eh-tab__gallery-item'));
      const imgEl    = lightbox.querySelector('img');
      const closeBtn = lightbox.querySelector('.eh-tab__lightbox-close');
      const backdrop = lightbox.querySelector('.eh-tab__lightbox-backdrop');

      if (!items.length || !imgEl || !closeBtn || !backdrop) return;
...
(function(){
  ...
  document.querySelectorAll('.tab-video-box').forEach(box=>{
    const thumb=box.querySelector('.tab-video-thumb');
    const raw=box.dataset.yt||box.dataset.ytUrl||box.getAttribute('data-yt-url')||box.getAttribute('data-tab-yt-url');
    const id=ytId(raw);
    if(!id||!thumb) return;
    bestThumb(id,u=>{ thumb.src=u; });
    const open=()=>{
    ...
    const btn=box.querySelector('.tab-video-play');
    btn&&btn.addEventListener('click',open,{passive:true});
    box.addEventListener('click',e=>{ if(e.target.closest('.tab-video-play')) return; open(); },{passive:true});
  });
})();
...
/* inline script 12 */
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('.eh-mob').forEach(function(block){

    /* SLIDER */
    (function(){
      const slider   = block.querySelector('.eh-mob__slider');
      if (!slider) return;

      const track    = slider.querySelector('.eh-mob__slider-track');
      const slides   = Array.from(slider.querySelectorAll('.eh-mob__slide'));
      const prevBtn  = slider.querySelector('.eh-mob__nav--prev');
      const nextBtn  = slider.querySelector('.eh-mob__nav--next');
      const dotsWrap = slider.querySelector('.eh-mob__dots');
    ...
    /* GALLERY + LIGHTBOX */
    (function(){
      const strip    = block.querySelector('.eh-mob__gallery-strip');
      const lightbox = block.querySelector('.eh-mob__lightbox');
      if (!strip || !lightbox) return;

      const items    = Array.from(strip.querySelectorAll('.eh-mob__gallery-item'));
      const imgEl    = lightbox.querySelector('img');
      const closeBtn = lightbox.querySelector('.eh-mob__lightbox-close');
      const backdrop = lightbox.querySelector('.eh-mob__lightbox-backdrop');
...
(function(){
  ...
  document.querySelectorAll('.mvid-box').forEach(box=>{
    const thumb=box.querySelector('.mvid-thumb');
    const raw=box.dataset.mvidUrl||box.dataset.ytUrl||box.dataset.yt||box.getAttribute('data-yt-url');
    const id=_mvidId(raw);
    if(!id||!thumb) return;
    _mvidBestThumb(id,u=>{ thumb.src=u; });
    const open=()=>{
      if(box.querySelector('iframe,video')) return;
      const f=document.createElement('iframe');
      f.src=`https://www.youtube.com/embed/${id}?autoplay=1&rel=0&modestbranding=1`;
    ...
    const btn=box.querySelector('.mvid-play');
    btn&&btn.addEventListener('click',open,{passive:true});
    box.addEventListener('click',e=>{ if(e.target.closest('.mvid-play')) return; open(); },{passive:true});
  });
})();
