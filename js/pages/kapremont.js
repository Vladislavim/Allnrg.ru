...
/* параллакс логотипа */
(function(){
  var root=document.querySelector('.proj');
  var brand=document.querySelector('.proj__brand');
...
/* модалка — изолированный нейминг, работает в WP */
(function(){
  function onReady(fn){
  ...
  onReady(function(){
    ...
    if(!modal) return;
    var back=modal.querySelector('.consult-modal__backdrop');
    var close=modal.querySelector('.consult-modal__close');
    var triggers=document.querySelectorAll('.js-open-consult');
...
/* inline script 8 */
/* открытие/закрытие модалки по кликам на кнопки в .proj-tab */
(function(){
  function onReady(fn){
    if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',fn,{once:true});}
    else{fn();}
  ...
  onReady(function(){
    ...
    var back=modal.querySelector('.consult-modal__backdrop');
    var close=modal.querySelector('.consult-modal__close');
    var triggers=document.querySelectorAll('.proj-tab .js-open-consult');
...
(function(){
  ...
  function lock(v){document.documentElement.style.overflow=v?'hidden':'';document.body.style.overscrollBehaviorY=v?'contain':'';document.body.style.touchAction=v?'none':'';}
  function openM(){modal.setAttribute('open','');modal.setAttribute('aria-hidden','false');lock(true);}
  function closeM(){modal.removeAttribute('open');modal.setAttribute('aria-hidden','true');lock(false);}
  document.addEventListener('click',e=>{const b=e.target.closest('.im-proj .im-btn');if(!b)return;e.preventDefault();openM();},{passive:false});
  back.addEventListener('click',closeM);close.addEventListener('click',closeM);
