...
(function(){
  ...
  prevBtn.addEventListener('click', () => { go(index - 1); autoplay(); });
  nextBtn.addEventListener('click', () => { go(index + 1); autoplay(); });
...
(function(){
  // premium gallery accordion + lightbox
...
/* inline script 7 */
(function(){
  const root = document.querySelector('.ko-tab');
  const brand = document.querySelector('.ko-tab__brand');
  if(!root || !brand) return;
  const speed = -0.5;
  function tick(){
    const y = -root.getBoundingClientRect().top;
    brand.style.setProperty('--shift', (y * speed) + 'px');
...
(function(){
  const slider = document.querySelector('.ko-tab__slider');
  if (!slider) return;

  const track = slider.querySelector('.ko-tab__slider-track');
  const slides = Array.from(slider.querySelectorAll('.ko-tab__slide'));
  const prevBtn = slider.querySelector('.ko-tab__nav--prev');
  const nextBtn = slider.querySelector('.ko-tab__nav--next');
  const dotsWrap = slider.querySelector('.ko-tab__dots');
...
(function(){
  const strip = document.querySelector('.ko-tab__gallery-strip');
  const items = strip ? Array.from(strip.querySelectorAll('.ko-tab__gallery-item')) : [];
  const lightbox = document.querySelector('.ko-tab__lightbox');
  if (!strip || !items.length || !lightbox) return;

  const imgEl = lightbox.querySelector('img');
  const closeBtn = lightbox.querySelector('.ko-tab__lightbox-close');
  const backdrop = lightbox.querySelector('.ko-tab__lightbox-backdrop');
...
(function(){
  document.addEventListener('DOMContentLoaded', function(){
    ...
    document.querySelectorAll('.ko-tab').forEach(function(block){

      /* SLIDER */
      (function(){
        const slider   = block.querySelector('.ko-tab__slider');
        if (!slider) return;
      ...
      /* GALLERY + LIGHTBOX + АВТОАККОРДЕОН */
      (function(){
        const strip    = block.querySelector('.ko-tab__gallery-strip');
        const lightbox = block.querySelector('.ko-tab__lightbox');
        if (!strip || !lightbox) return;

        const items    = Array.from(strip.querySelectorAll('.ko-tab__gallery-item'));
        const imgEl    = lightbox.querySelector('img');
        const closeBtn = lightbox.querySelector('.ko-tab__lightbox-close');
        const backdrop = lightbox.querySelector('.ko-tab__lightbox-backdrop');
        ...
        function setActive(idx){
          accIndex = (idx + items.length) % items.length;
          items.forEach(function(it, i){
            it.classList.toggle('is-active', i === accIndex);
          });
        }

        function startAccord(){
          clearInterval(accTimer);
          accTimer = setInterval(function(){
            setActive(accIndex + 1);
          }, AUTOTIME);
        }
        ...
        // клик по элементу: активируем + открываем
        ...
        // пауза при ховере на галерею
        strip.addEventListener('mouseenter', stopAccord);
        strip.addEventListener('mouseleave', startAccord);
