...
(function(){
  ...
  const openBtns = document.querySelectorAll('.lab-offer__btn'); // кнопка "Связаться"
...
/* inline script 12 */
(function(){
  const modal   = document.getElementById('labtcm-root');
  if (!modal) return;

  const backdrop = modal.querySelector('.labtcm__back');
  const closeBtn = modal.querySelector('.labtcm__close');
  const openBtns = document.querySelectorAll('.lab-tab-offer__btn'); // кнопка "Связаться"
