...
(function () {
  ...
  const targetCard = grid.children[8];
  if (!targetCard) return;

  function placeLogo() {
    const innerRect = inner.getBoundingClientRect();
    const cardRect  = targetCard.getBoundingClientRect();
    const left = (cardRect.right - innerRect.left) + OFFSET_X;
    const top  = (cardRect.top - innerRect.top) + OFFSET_Y;
  ...
  function applyParallax() {
    const shift = (getScrollTop() - sectionStart) * SPEED;
    logo.style.transform = 'translate3d(0,' + shift + 'px,0)';
  }

  const onScroll = () => applyParallax();
  const onResize = () => { placeLogo(); updateSectionStart(); applyParallax(); };
...
/* inline script 6 */
/* Параллакс для планшета: логотип и верхняя картинка двигаются по Y */
(function(){
  const section = document.querySelector('.srv-tab');
  const inner   = document.querySelector('.srv-tab__inner');
  const logo    = document.querySelector('.srv-tab__logo');
  const heading = document.querySelector('.srv-tab__heading');
  if(!section || !inner || !logo || !heading) return;

  const mq = window.matchMedia('(min-width: 768px) and (max-width: 1024px)');
  const SPEED = -0.35; // сила эффекта
  ...
  function apply(){
    const dy = (getScrollTop() - sectionStart) * SPEED;
    // один и тот же var для двух элементов (шапка движется чуть медленнее через CSS-scale/доп. множитель)
    logo.style.setProperty('--st-dy', dy + 'px');
    heading.style.setProperty('--st-dy', (dy * 0.6) + 'px');
  }

  function tick(){
    apply();
    rafId = requestAnimationFrame(tick); // плавность и инерция iOS
  }
  ...
  function bind(){
    ...
    window.addEventListener('resize', ()=>{ updateSectionStart(); apply(); }, {passive:true});
    window.addEventListener('orientationchange', ()=>{ updateSectionStart(); apply(); });
    bound = true;
  }
  ...
  function unbind(){
    ...
    window.removeEventListener('orientationchange', updateSectionStart);
    cancelAnimationFrame(rafId);
    rafId = 0;
    bound = false;
  }

  function gate(){ mq.matches ? bind() : unbind(); }
  gate();
...
/* inline script 10 */
(function(){
  const section = document.querySelector('.srv-mob');
  const inner   = document.querySelector('.srv-mob__inner');
  const logo    = document.querySelector('.srv-mob__logo');
  const heading = document.querySelector('.srv-mob__heading');
  if(!section || !inner || !logo || !heading) return;
  ...
  function apply(){
    const dy = (getScrollTop() - sectionStart) * SPEED;
    logo.style.setProperty('--sm-dy', dy + 'px');
    heading.style.setProperty('--sm-dy', (dy * 0.6) + 'px');
  }

  window.addEventListener('scroll', apply, {passive:true});
  window.addEventListener('resize', () => { updateSectionStart(); apply(); });
